...
class FormConfigure extends React.Component {
    ...
    handleDelete = (idx) => (e) => {
        ...
        sections[secid].questions[idx].suggested_answers = sections[secid].questions[idx].suggested_answers.filter(
            (item, i) => index != i
        );
        sections[secid].questions[idx].suggested_jump = sections[secid].questions[idx].suggested_jump.filter(
            (item, i) => index != i
        );
        // const sectionss = this.state.sections[secid].questions[idx].suggested_answers.filter((item, i) => index != i)
        sections[secid].questions[idx].suggested_answers = sections[secid].questions[idx].suggested_answers.map(
            (item, id) => {
                return {
                    id: id,
                    value: item.value,
    ...
    addAnswer = (i) => (e) => {
        ...
        let sections = [...this.state.sections];
        sections[secid].questions[i].suggested_answers = this.state.sections[secid].questions[
            i
        ].suggested_answers.concat(['']);
        this.setState({ sections });
    };
            ...
            if (e.target.dataset.name === 'suggested_answers' || e.target.dataset.name === 'suggested_jump') {
                ...
                if (e.target.dataset.name === 'suggested_jump') {
                    sections[secid].questions[e.target.dataset.id].suggested_jump[e.target.dataset.idy] = {
                        answer: sections[secid].questions[e.target.dataset.id].suggested_answers[e.target.dataset.idy]
                            .value,
    ...
    jumpOptions = () => {
        ...
        this.state.sections
            ...
            .map((q, i) => options.push({ value: q.section, label: q.section, id: i }));
        return options;
    };
    Question_jumpOptions = (id) => {
        ...
        this.state.sections[id].questions
            ...
            .map((q, i) => options.push({ value: q.question, label: q.question, id: i }));
        return options;
    };
    ...
    render() {  
        ...
        this.state.sections &&
            this.state.sections
                .filter((sec, key) => sec.related === 'true')
                .map((q, i) => jumpOpt.push({ value: q.section, label: q.section, id: i }));
        if (isPreview) {
            return (
                <Modal
                    ...
                    <div className="row mt-3 text-center">
                        ...
                        <div className="col col-sm-2">
                            <button
                                ...
                                disabled={sectionLength === id ? false : true}
                            >
                                Add Section
                            </button>
                        </div>
