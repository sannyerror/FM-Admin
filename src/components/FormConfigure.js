...
class FormConfigure extends React.Component {
	...
	componentDidMount = async () => {
		...
		const org_type = this.props.organizationsList.find(
		...
		if (!Array.isArray(response.response)) {
			...
			let sectionResponse =
				...
				response.response.find((section) => section.section === "Outcomes");
			let index =
				Array.isArray(response.response) &&
				response.response.findIndex(
												...
												ques.suggested_jump.length > 0
													? ques.suggested_jump.map((s, i) => {
															return {
																...s,
																question_jumpto:
																	Array.isArray(s.question_jumpto) &&
																	s.question_jumpto.length > 0
																		? s?.question_jumpto.filter(
																				(j) => j != secname
																		  )
																		: [],
															};
									...
									<div className="form-check form-check-inline col-sm-1 ml-3">
										<input
											...
											data-secname={
												...
												this.state.sections[id].section
											}
